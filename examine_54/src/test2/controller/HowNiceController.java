...
@Controller()
public class HowNiceController
{
	...
	@RequestMapping(value = "/hownice", method = RequestMethod.GET)
	public ModelAndView getHowNicePage()
	{
		try (ClassPathXmlApplicationContext context = new ClassPathXmlApplicationContext(CONTEXT_FILE))
		{
			String page = (String)context.getBean("HOW_NICE_PAGE");
			return new ModelAndView(page);
		}
	}
	
	@RequestMapping(value = "/doHowNice", method = RequestMethod.POST)
	public ModelAndView doHowNice(@Valid LikeModel likeModel, BindingResult bindingResult)
	{
		try (ClassPathXmlApplicationContext context = new ClassPathXmlApplicationContext(CONTEXT_FILE))
		{
			String successPage = (String)context.getBean("HOW_NICE_SUCCESS_PAGE");
			...
			try { model = database.findByName(likeModel.getName()); }
