...
public class QuitController
{
	...
	public ModelAndView getQuitPage()
	{
		try (ClassPathXmlApplicationContext context = new ClassPathXmlApplicationContext(CONTEXT_FILE))
		{
			String quitPage = (String)context.getBean("QUIT_PAGE");
			return new ModelAndView(quitPage);
		}
	}
	...
	public ModelAndView doPay(@Valid HelloModel helloModel, BindingResult bindingResult)
	{
		try (ClassPathXmlApplicationContext context = new ClassPathXmlApplicationContext(CONTEXT_FILE))
		{
			String successPage = (String)context.getBean("QUIT_SUCCESS_PAGE");
			...
			try	{ feeModel = quitFee(feeModel, fee); }
			catch (NullAccountException | FeeNotPaidException exception)
			{
	...
	private FeeModel quitFee(FeeModel feeModel, Fee fee) throws Exception, FeeNotPaidException 
	{
