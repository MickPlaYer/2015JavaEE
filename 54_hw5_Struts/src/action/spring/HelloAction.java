...
{
	private final String EXCEPTION_MESSAGE = "resources.ExceptionMessage";
	...
	{
		// 取得資源檔，內含錯誤提示文字
		ResourceBundle resource = ResourceBundle.getBundle(EXCEPTION_MESSAGE);
		if (_helloForm.getName().length() != 0)
		{
			...
			if (!isLetter && !isCharacter)
				this.addFieldError(FIELD_ERROR_ERROR_NAME, resource.getString(ERROR_CHAR_ONLY));
		}
		else
		{
			// 規則：名稱不能為空
			this.addFieldError(FIELD_ERROR_NULL_NAME, resource.getString(ERROR_EMPTY));
		}
		// 呼叫父項validate函式
		super.validate();
	}
